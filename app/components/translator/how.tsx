  ...
  const cards = [
    <article className={styles.card} key="card-1">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Загрузка</h3>
        <p className={styles.cardText}>
          Вы присылаете ссылку или файл.
        </p>

        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/local-1.png" alt="" />
        </div>
    ...
    <article className={styles.card} key="card-2">
      ...
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/local-2.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 2</p>
        <h3 className={styles.cardTitle}>Адаптация</h3>
        <p className={styles.cardText}>
          Переводим текст, согласуем терминологию, клонируем голос.
        </p>
    ...
    <article className={styles.card} key="card-3">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Генерация</h3>
        <p className={styles.cardText}>
          ИИ создаёт новую аудиодорожку, заменяет мимику (LipSync) и синхронизирует речь.
        </p>
        
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/local-3.png" alt="" />
        </div>
    ...
    <article className={styles.card} key="card-4">
      {/* Десктопная картинка (слева) */}
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/local-4.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 4</p>
        <h3 className={styles.cardTitle}>Автопостинг</h3>
        <p className={styles.cardText}>
         Видео автоматически публикуются на ваши международные каналы.
        </p>
    ...
    <article className={styles.card} key="card-5">
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 5</p>
        <h3 className={styles.cardTitle}>Автоматизация</h3>
        <p className={styles.cardText}>
          Всё можно вынести в Telegram-бот: загрузка → локализация → готовый файл → публикация.
        </p>
        
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/local-5.png" alt="" />
        </div>
    ...
    <section id={id} className={styles.section}>
      <div className={styles.inner}>
        <HeaderWrapper
          ...
          titleAccent={<span className="color-white">— всего за 5 шагов</span>}
          align="center"
          withBr={false}
        />
