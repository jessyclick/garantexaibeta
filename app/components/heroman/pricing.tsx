...
import styles from '../socmedia/tariffs.module.scss';
...
const Pricing = () => {
  const plans = [
    {
      id: 'start',
      title: '#Старт',
      price: ' 7 500₽ единоразово',
      forwho: 'Малый бизнес, блогеры',
      under: 'Тех. поддержка: от 10 000 ₽/мес',
      gradient: 'firstGradient',
      features: [
        { text: ' Разработка концепции и характера', checked: true },
        { text: '3D-моделирование и рендеринг', checked: true },
        { text: 'Набор поз для сайта и баннеров', checked: true },
        { text: 'Исходные файлы в собственность', checked: true },
      ],
    },
    {
      id: 'business',
      title: '#Голос и движение',
      price: ' от 2 500₽ базовая настройка',
      forwho: 'Средний бизнес, эксперты',
      under: 'Тех. поддержка: от 15 000 ₽/мес',
      gradient: 'secondGradient',
      features: [
        { text: 'Клонирование вашего голоса (1 день)', checked: true },
        { text: 'Настройка мимики и липсинка', checked: true },
        { text: 'Далее: 500₽ за каждые 5 сек. готового видео', checked: true },
      ],
    },
    {
      id: 'corp',
      ...
      features: [
        { text: 'Автопостинг в соцсети', checked: true },
        { text: 'Регулярная генерация Reels/Shorts', checked: true },
        { text: 'Готовые сценарии и идеи', checked: true },
  ...
  return (
    <section className={styles.seoAutomation}>
      <div className={styles.container}>
        <HeaderWrapper
                            ...
                            href=""
                            kicker="Тарифы"
                            title="Соберите своего героя"
                            titleAccent=""
                          />

        <div className='grid grid-cols-3 gap-4'>
          {plans.map((plan) => (
            <div key={plan.id} className={`${styles.plan} ${styles[plan.id]} ${plan.gradient}`}>
              <h4 className={styles.planTitle}>{plan.title}</h4>

              <ul className={styles.featuresList}>
                {plan.features.map((feature, index) => (
                  <li key={index} className={feature.checked ? styles.checked : ''}>
