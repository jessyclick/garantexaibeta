  ...
  const cards = [
    <article className={styles.card} key="card-1">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Мониторинг источников </h3>
        <p className={styles.cardText}>
        Вы даёте список ресурсов,
        или мы подбираем его за вас. <br />   
        Система:
        <ul>
            <li>анализирует источники каждые 5 минут,</li>
            <li>отслеживает ключевые темы,</li>
            <li>фиксирует появление новых инфоповодов.</li>
        </ul>
      <br />
        Это ваш личный новостной радар.
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/ne-1.png" alt="" />
        </div>
    ...
    <article className={styles.card} key="card-2">
      ...
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/ne-2.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 2</p>
        <h3 className={styles.cardTitle}>Анализ и отбор новостей</h3>
        <p className={styles.cardText}>
          
        ИИ оценивает свежую новость по нескольким параметрам:

    <ul>
        <li>тема <span>(крипта, финансы, IT, недвижимость, бизнес и др.)</span></li>
        <li>ключевые слова</li>
        <li>география</li>
        <li>значимость события</li>
        <li>уровень шума</li>
        <li>наличие «мусора» (реклама, казино, clickbait, фейки)</li>
    </ul>

  
        В финальный поток попадают только полезные и релевантные материалы.
    ...
    <article className={styles.card} key="card-3">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Глубокий рерайт и генерация</h3>
        <p className={styles.cardText}>
    
    <p>Нейросеть полностью переписывает оригинальную новость:</p>
    <ul>
        <li>новый заголовок <span>(кликабельный / SEO / нейтральный)</span></li>
        <li>новая структура текста</li>
        <li>переработанные факты</li>
        <li>добавленные детали и контекст</li>
        <li>уникальность 90–100%</li>
    </ul>
      <br />
    <p>Также можно настроить:</p>
    <ul>
        <li>длину текста</li>
        <li>стиль</li>
        <li>плотность ключевых слов</li>
        <li>формат <span>(новость / дайджест / аналитика / короткий пост)</span></li>
    </ul>
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/ne-3.png" alt="" />
        </div>
    ...
    <article className={styles.card} key="card-4">
      {/* Десктопная картинка (слева) */}
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/ne-4.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 4</p>
        <h3 className={styles.cardTitle}>Автопубликация</h3>
        <p className={styles.cardText}>
          <p>Готовый материал автоматически уходит:</p>
    <ul>
        <li>на ваш сайт</li>
        <li>в Telegram-канал</li>
        <li>в VK / OK</li>
        <li>в зарубежные соцсети*</li>
        <li>в Google Docs (если нужна премодерация)</li>
    </ul>
      <br />
    <p>Публикация может быть:</p>
    <ul>
        <li>мгновенная</li>
        <li>по расписанию</li>
        <li>через вашу модерацию в телеграмм боте</li>
    </ul>
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
    ...
    <section id={id} className={styles.section}>
      <div className={styles.inner}>
        <HeaderWrapper
          ...
          title={<span className="color-white">От события в мире → до публикации
на ваших ресурсах </span>}
          titleAccent={<span>— за 5 минут</span>}
          align="center"
          withBr={false}
        />
