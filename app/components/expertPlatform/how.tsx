  ...
  const cards = [
    <article className={styles.card} key="card-1">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Оцифровка экспертизы</h3>
        <p className={styles.cardText}>
          Мы создаем «Базу знаний» вашего бренда. Загружаем в AI ваши кейсы, техническую документацию, Tone-of-Voice и боли клиентов. ИИ учится думать, как вы.
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/how-1.png" alt="" />
        </div>
        <CTAButton href="#" icon={btnIcon}>Получить консультацию</CTAButton>
      </div>
    ...
    <article className={styles.card} key="card-2">
      ...
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/how-2.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 2</p>
        <h3 className={styles.cardTitle}>Стратегия и Сценарий</h3>
        <p className={styles.cardText}>
          ИИ-сценарист разрабатывает контент-план. Он определяет тему, целевую аудиторию, «хук» (цепляющее начало) и целевое действие для каждой статьи.
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/how-2.png" alt="" />
    ...
    <article className={styles.card} key="card-3">
      <div className={styles.cardBody}>
        ...
        <h3 className={styles.cardTitle}>Производство и Адаптация</h3>
        <p className={styles.cardText}>
          Система пишет статью. Затем адаптирует её под форматы разных площадок: делает провокационный заголовок для VC, технический для Хабра и нативный для Дзена.
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/how-3.png" alt="" />
        </div>
        <CTAButton href="#" icon={btnIcon}>Получить консультацию</CTAButton>
      </div>
      {/* Десктопная картинка (справа) */}
    ...
    <article className={styles.card} key="card-4">
      {/* Десктопная картинка (слева) */}
      <div className={`${styles.cardPreview} hidden md:block`}>
        <img src="/assets/how-4.png" alt="" />
      </div>
      <div className={styles.cardBody}>
        <p className={styles.cardBadge}>Этап 4</p>
        <h3 className={styles.cardTitle}>Публикация</h3>
        <p className={styles.cardText}>
          Вы получаете готовые материалы в Google Docs или Telegram. По запросу мы берем на себя верстку и постинг под ключ.
        </p>
        {/* Мобильная картинка (перед кнопкой) */}
        <div className={`${styles.cardPreview} block md:hidden`}>
          <img src="/assets/how-4.png" alt="" />
